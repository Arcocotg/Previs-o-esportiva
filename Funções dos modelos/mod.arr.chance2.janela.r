mod.arr.chance2.janela <- function(tab, comp.nome = NULL, plot = F) {
  
  # Criar o nome do arquivo dinamicamente
  arquivo_saida <- paste0("prev.mod.arr.chance2.janela", comp.nome, ".csv")
  ...
  ### constroi o vetor indice 
  {
    ...
    sem_atual <- tab_sem$semestre[1]
    k=1
    
    for (j in 1:nrow(tab_sem)) {
      
      if(sem_atual == tab_sem$semestre[j]){
        tab_sem$indice0[j] <- k
      }else{
        k <- k+1
        sem_atual <- tab_sem$semestre[j]
        tab_sem$indice0[j] <- k
      }
    }
    
    times0 <- sort(unique(c(tab_sem$mand,tab_sem$vist)))
  }
  
  
  for (o in 1:length(unique(tab_sem$indice0))) {
    
    tab2 <- tab_sem[tab_sem$indice0 == o,]
    ########################################
    ...
    for (n in 2:nrow(tab2)){
      ...
      bc <- tryCatch(
        { glm(g ~ matriz, family = poisson, weights = w) },
