mod.arr.SD0.rodada <- function(tab, comp.nome = NULL, nr = 2, plot = F) {
  
  # Criar o nome do arquivo dinamicamente
  arquivo_saida <- paste0("prev.mod.arr.SD0.rodada", comp.nome, ".csv")
  ...
  #fazer previsões fragmentadas considerando as rodadas anteriores
  {
    for (l in (nr+1):length(unique(tab_rod$indice))) {
      ...
      for (p in 1:nj2) {
        ...
        ES <- matriz_alpha[p,]%*%parametros1[,3]
        ED <- matriz_beta[p,]%*%parametros1[,2]
