mod.ufmg.comsimul.ano <- function(tab, p = 5, comp.nome = NULL, n = 10000, plot = F) {
  
  # Criar o nome do arquivo dinamicamente
  arquivo_saida <- paste0("prev.mod.ufmg.comsimul.ano", comp.nome, ".csv")
  ano_atual <- tab$ano[1]
  ...
  for (i in 1:nrow(tab)) {
    ...
    # Atualização baseada no resultado do jogo
    if (jogo$resul == "vm") {  # Vitória mandante
      PM <- ((p * PM) + (vist$R * c(1, 0, 0))) / (p + vist$R)
      PV <- ((p * PV) + ((1 - mand$R) * c(1, 0, 0))) / (p + (1 - mand$R))
      mand$Rc <- mand$Rc + 3
    } else if (jogo$resul == "vv") {  # Vitória visitante
